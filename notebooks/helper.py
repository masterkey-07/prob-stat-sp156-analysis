...
def plot_bar_chart_by_month(df, title):
    df["data de abertura"] = pd.to_datetime(df["data de abertura"])

    df['month'] = df["data de abertura"].dt.to_period('M')

    count_data = df.groupby(['month', "tema", "assunto"]).size().reset_index(name='count')

    # Plotando o gráfico de barras
    plt.figure(figsize=(12, 8))
    sns.barplot(data=count_data, x='month', y='count', hue="assunto", palette='muted')
    plt.title(title)
    plt.xlabel('Mês')
    plt.ylabel('Contagem')
    plt.legend(title='Assunto')
